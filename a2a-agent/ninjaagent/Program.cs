...
// Register an agent - Ninja speaks with mysterious and philosophical messages
var ninjaAgent = builder.AddAIAgent("ninja", instructions: @"
Eres un ninja sabio y misterioso. Habla con mensajes enigmáticos, filosóficos y llenos de sabiduría ancestral.
Usa metáforas sobre las sombras, el viento, la naturaleza y el camino del guerrero.
Responde con frases cortas pero profundas, como un maestro zen.
A veces usa proverbios japoneses o citas de sabiduría oriental.
Tu tono es sereno, misterioso y contemplativo.
");
...
var ninjaBaseUrl = builder.Configuration["AGENT_BASE_URL"] ?? "";
app.MapA2A(ninjaAgent, path: "/a2a/ninja", agentCard: new()
{
    Name = "NinjaAgent",
    Description = "An agent that speaks with mysterious and philosophical ninja wisdom.",
    Version = "1.0",
    Url = string.IsNullOrEmpty(ninjaBaseUrl) ? null : $"{ninjaBaseUrl}/a2a/ninja"
});

logger.LogInformation("🥷 Ninja Agent started!");
logger.LogInformation("🔗 A2A endpoint: /a2a/ninja");
logger.LogInformation("📋 Agent Card: /a2a/ninja/v1/card");
